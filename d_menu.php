...
?>
  ...
  <div class="row mx-auto p-2">
     ...
     <?php
        ...
        $data = mysqli_query($db,"select m.*,j.jenis from menu m, jenis j where m.id_jenis=j.id_jenis and m.nama_menu like '%".$cari."%'");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"select m.*,j.jenis from menu m, jenis j where m.id_jenis=j.id_jenis and m.nama_menu like '%".$cari."%' limit $halaman_awal, $batas");
        ...
        $data = mysqli_query($db,"select m.*,j.jenis from menu m, jenis j where m.id_jenis=j.id_jenis");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"select m.*,j.jenis from menu m, jenis j where m.id_jenis=j.id_jenis limit $halaman_awal, $batas");
        $nomor = $halaman_awal+1;
                              ...
                              <div class="row">
                                  <div class="col-md-12">
                                      <input type="submit" name="pesan" class="btn btn-primary btn-sm" value='Pesan sekarang'>
                                       ...
                                       <a href="retail/s_wishlist.php?id=<?php echo $data['id_menu']?>" class="btn btn-outline-danger btn-sm fas fa-heart pt-2" ></a>
                                        <a href="detailmenu.php?id=<?php echo $data['id_menu']?>" class="btn btn-outline-danger btn-sm " >Detail</a>
                                       </div>
                                    </div>
