...
<?php include "atas.php"?>
  ...
  <div class="row mx-auto">
     ...
     $id=$hasil['id_user'];
            ...
            if (isset($_GET['kirim'])) {
                $cari = $_GET['cari'];
                $query = "SELECT * FROM user WHERE type_user='penjual'  AND status='aktif' AND id_user !='$id' AND nama like '%" . $cari . "%'";
            } else {
                // jika tombol cari tidak di klik menjalankan query menampilkan semua data
                 $query="SELECT * FROM user WHERE type_user='penjual' AND id_user !='$id' AND status='aktif'";
            }
