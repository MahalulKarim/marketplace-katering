  ...
  <div class="row mx-auto">
     ...
     $id_user=$hasil['id_user'];
       if (isset($_GET['kirim'])) {
        ...
        $data = mysqli_query($db,"SELECT p.*,w.id_wishlist FROM wishlist w, menu p  WHERE w.id_menu=p.id_menu AND w.id_user='$id_user' AND p.nama_menu like '%" . $cari . "%'");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"SELECT p.*,w.id_wishlist FROM wishlist w, menu p  WHERE w.id_menu=p.id_menu AND w.id_user='$id_user' AND p.nama_menu like '%" . $cari . "%' limit $halaman_awal, $batas");
        ...
        $data = mysqli_query($db,"SELECT p.*,w.id_wishlist FROM wishlist w, menu p  WHERE w.id_menu=p.id_menu AND w.id_user='$id_user'");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"SELECT p.*,w.id_wishlist FROM wishlist w, menu p  WHERE w.id_menu=p.id_menu AND w.id_user='$id_user' limit $halaman_awal, $batas");
        $nomor = $halaman_awal+1;
