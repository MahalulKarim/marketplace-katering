...
<?php include "atas.php"?>
  ...
  <div class="row mx-auto">
     ...
     $idu=$hasil['id_user'];
        ...
        $data = mysqli_query($db,"SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual'  AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack' AND p.nama_menu like '%" . $cari . "%'");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual'  AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack' AND p.nama_menu like '%" . $cari . "%' limit $halaman_awal, $batas");
        ...
        $data = mysqli_query($db,"SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack'");
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $data_barang = mysqli_query($db,"SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack' limit $halaman_awal, $batas");
        $nomor = $halaman_awal+1;
        ...
        //         $query = "SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual'  AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack' AND p.nama_menu like '%" . $cari . "%'";
        //     } else {
        //         // jika tombol cari tidak di klik menjalankan query menampilkan semua data
        //          $query="SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND p.id_user!='$idu' AND j.id_jenis=p.id_jenis AND j.jenis='Snack'";
        //     }
                          ...
                          <div class="col-md-6">
                            <p class="card-title mb-2"><b><?php echo $data['nama_menu'] ?></b></p>
                             ...
                             <?php $id=$data['id_menu'];
                              ...
                              <div class="row">
                                  <div class="col-md-12">
                                      <a href="detailmenu.php?id2=<?php echo $data['id_menu']?>" class="btn btn-primary btn-sm">Pesan Sekarang</a>
                                       <div class="btn-group pt-2">
