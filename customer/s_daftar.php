        ...
        if (isset($_POST['daftar'])) {
            ...
            $tmp = $_FILES['file']['tmp_name'];
              ...
              if(empty($nama)||empty($username)||empty($password)||empty($alamat)||empty($email)||empty($no_hp)||empty($tgl_lahir)||empty($jk)){
                        ...
                        alert('Data harus diisi semua!');window.location='f_daftar.php'</script>
                                            ";
                    }else{
                            ...
                            if (mysqli_num_rows($result) < 1) {
                                ...
                                if (move_uploaded_file($tmp, $path)) {
                                    $query = "INSERT INTO `user` SET type_user='$type_user',nama='$nama',username='$username',password='$password',email='$email',alamat='$alamat',jk='$jk',tgl_lahir='$tgl_lahir',no_hp='$no_hp',foto='$fotobaru',status='aktif'";
                                    $result = mysqli_query($db, $query); 
                                    	echo "<script type='text/javascript'>
                                    ...
                                    $query = "INSERT INTO `user` SET type_user='$type_user',nama='$nama',username='$username',password='$password',email='$email',alamat='$alamat',jk='$jk',tgl_lahir='$tgl_lahir',no_hp='$no_hp',status='aktif'";
                                    $result = mysqli_query($db, $query);
