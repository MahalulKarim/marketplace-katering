<?php include "atas.php"?>
  ...
  <div class="row mx-auto">
     ...
     <?php
        ...
        //     if (isset($_GET['kirim'])) {
        //         $cari = $_GET['cari'];
        //         $query = "SELECT p.*,u.nama FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND p.id_jenis=j.id_jenis AND j.jenis='Lainya' AND p.nama_menu like '%" . $cari . "%'";
        //     } else {
        //         // jika tombol cari tidak di klik menjalankan query menampilkan semua data
        //          $query="SELECT p.*,u.nama FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND  p.id_jenis=j.id_jenis AND j.jenis='Lainya' AND u.type_user='penjual'";
        //     }
     ...
     if (isset($_GET['kirim'])) {
                ...
                $data = mysqli_query($db,"SELECT p.*,j.jenis FROM menu p, jenis j, user u WHERE p.id_jenis=j.id_jenis AND u.id_user=p.id_user AND u.status='aktif' AND j.jenis='Lainya' AND p.nama_menu like '%" . $cari . "%'");
                $jumlah_data = mysqli_num_rows($data);
                $total_halaman = ceil($jumlah_data / $batas);
         
                $data_barang = mysqli_query($db,"SELECT p.*,j.jenis FROM menu p, jenis j, user u WHERE p.id_jenis=j.id_jenis AND u.id_user=p.id_user AND u.status='aktif' AND j.jenis='Lainya' AND p.nama_menu like '%" . $cari . "%' limit $halaman_awal, $batas");
              ...
              $batas = 6;
                ...
                $data = mysqli_query($db,"SELECT p.*,j.jenis FROM menu p, jenis j, user u WHERE  u.id_user=p.id_user AND p.id_jenis=j.id_jenis AND u.status='aktif' AND j.jenis='Lainya'");
                $jumlah_data = mysqli_num_rows($data);
                $total_halaman = ceil($jumlah_data / $batas);
         
                $data_barang = mysqli_query($db,"SELECT p.*,j.jenis FROM menu p, jenis j, user u WHERE  u.id_user=p.id_user AND p.id_jenis=j.id_jenis AND u.status='aktif' AND j.jenis='Lainya' limit $halaman_awal, $batas");
                $nomor = $halaman_awal+1;
