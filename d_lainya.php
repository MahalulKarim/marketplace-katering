  ...
  <div class="row mx-auto">
     ...
     $batas = 6;
        ...
        $next = $halaman + 1;
          ...
          $query="SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND j.id_jenis=p.id_jenis AND j.jenis='Lainya'";
        
        $data = mysqli_query($db,$query);
        $jumlah_data = mysqli_num_rows($data);
        $total_halaman = ceil($jumlah_data / $batas);
 
        $query1="SELECT p.*,u.nama,j.* FROM menu p, user u, jenis j WHERE p.id_user=u.id_user AND u.type_user='penjual' AND j.id_jenis=p.id_jenis AND j.jenis='Lainya' LIMIT $halaman_awal, $batas";
